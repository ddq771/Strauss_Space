# Merlin 1D sea-level — visual model for Strauss Space
...
- `../../Assets/Models/Merlin1D/Merlin1D_Engine.fbx`: Unity asset.
- `Merlin1D_Engine.blend`: editable Blender source with preview studio.
- `Merlin1D_Engine_preview.png`: render of the exported model's source geometry.
- `build_merlin1d.py`: model generator, using `../engine_asset.py`.
...
Refresh Unity, then drag `Merlin1D_Engine.fbx` from its Project folder into a
...
```sh
blender --background --threads 8 --python ArtSource/Merlin1D/build_merlin1d.py
```
